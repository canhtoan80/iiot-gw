...
#### ✅ I.2 — South-facing MQTT Broker lựa chọn: MQTTnet.Server vs Mosquitto vs NanoMQ — Phân tích và Quyết định kiến trúc

**Bối cảnh đã xác lập từ đánh giá trước:**

Trong kịch bản G3 tách máy có thiết bị MQTT Native (Luồng A), Gateway cần một **South-facing MQTT Broker** chạy ngay tại Tầng 2A — nhận dữ liệu từ thiết bị trường độc lập với EMQX OT (Tầng 2B). Khi EMQX OT down, thiết bị vẫn publish được vào Gateway, firmware vẫn xử lý và buffer. EMQX OT chỉ là uplink destination — North-facing broker.

Câu hỏi còn lại: **South-facing broker nào phù hợp nhất?** Ba ứng viên cần đánh giá kỹ: `MQTTnet.Server`, `Mosquitto`, và `NanoMQ`.

---

**Phân tích chi tiết từng ứng viên:**

##### Ứng viên 1 — `MQTTnet.Server` (embedded trong .NET process)

`MQTTnet` là thư viện .NET thuần túy (MIT license, thuộc .NET Foundation), cung cấp cả MQTT client và MQTT server trong cùng một NuGet package. Xử lý ~150.000 messages/giây trong benchmark nội bộ (client và server cùng process, Intel i7 8700K), hỗ trợ MQTT 3.1/3.1.1/5.0, TCP+TLS và WebSocket.

| Tiêu chí | Đánh giá | Chi tiết |
|---|---|---|
| Tích hợp .NET | ✅ Tốt nhất | Cùng process, cùng DI container, cùng Serilog, không IPC |
| RAM footprint | ✅ Thấp | ~10–20MB thêm vào .NET runtime đang chạy |
| Dependency | ✅ Không cần cài thêm | Chỉ một NuGet package |
| MQTT 5.0 | ✅ Đầy đủ | Hỗ trợ hoàn chỉnh |
| TLS/ACL | ⚠ Tự implement | Không có built-in ACL rule engine như broker standalone |
| Bridge OT→IT | ❌ Không có | Phải tự implement — không cần vì EMQX OT đảm nhận |
| Management API | ❌ Không có | Không cần — firmware tự quản lý nội bộ |
| Độ trưởng thành | ⚠ Broker mode ít dùng | Client mode phổ biến; Server mode ít deployment production thực tế |
| Lifecycle coupling | ⚠ Gắn với firmware | Broker crash = firmware crash; firmware restart = broker restart |
| Fault isolation | ❌ Kém | Broker lỗi không cô lập được với pipeline firmware |
| Restart độc lập | ❌ Không thể | Không thể restart broker mà không restart firmware |
| Giám sát | ⚠ Phải tự làm | Cần implement metric endpoint riêng |
| Systemd management | ❌ Không | Không có unit file riêng, không có restart policy riêng |

**Rủi ro quan trọng — Lifecycle Coupling:** Khi `MQTTnet.Server` embedded trong cùng process firmware:
- Memory leak trong broker → toàn bộ firmware bị ảnh hưởng
- Broker có bug → phải deploy lại firmware (OTA) thay vì chỉ restart broker
- Firmware đang nâng cấp OTA → broker cũng down trong thời gian đó → thiết bị MQTT mất nơi publish
- Không thể restart broker độc lập để xử lý hung connection mà không dừng thu thập

---

##### Ứng viên 2 — Mosquitto (Eclipse, C, single-threaded)

Mosquitto là MQTT broker phổ biến nhất thế giới, booting footprint khoảng 200KB, không hỗ trợ multi-threading hoặc clustering native. Thiết kế đơn giản và sạch, chạy như single-threaded daemon với epoll support — phù hợp thiết bị nhúng có tài nguyên thấp.

| Tiêu chí | Đánh giá | Chi tiết |
|---|---|---|
| Độ trưởng thành | ✅ Cao nhất | Ra đời 2009, donate Eclipse Foundation, >10k GitHub stars |
| Stability | ✅ Đã chứng minh | Triển khai production hàng triệu node trên toàn thế giới |
| RAM footprint | ✅ Cực thấp | ~200KB startup, RAM khi idle <5MB |
| Fault isolation | ✅ Process riêng | Crash broker không kéo firmware down |
| Restart độc lập | ✅ Có | `systemctl restart mosquitto` không ảnh hưởng firmware |
| Systemd integration | ✅ Tốt | Unit file chuẩn, restart policy, logging journald |
| ACL | ✅ Có | File-based ACL rules, username/password |
| TLS | ✅ Có | TLS/mTLS chuẩn |
| MQTT 5.0 | ✅ Có | Hỗ trợ từ v2.0+ |
| Multi-threading | ❌ Không | Single-threaded — bottleneck khi nhiều client concurrent |
| Throughput | ⚠ Giới hạn | Fan-out scenario: Mosquitto chỉ đạt ~37.300 msg/s ổn định, trong khi NanoMQ đạt 50.000 msg/s trong cùng điều kiện. |
| Data persistence | ⚠ Giới hạn | SQLite/file caching nhưng bị giới hạn bởi single-thread — broker stop responding khi quá nhiều disk I/O đồng thời |
| .NET integration | ⚠ IPC qua TCP | `MQTTnet` client kết nối TCP localhost — thêm một TCP round-trip |
| Dependency | ⚠ External process | Phải cài `mosquitto` package riêng |

**Giới hạn thực tế với Gateway SME 50–200 điểm đo:** Với giả thiết tối đa 50 thiết bị MQTT Native, mỗi thiết bị publish 1–10 msg/s → tổng ~500 msg/s → hoàn toàn trong khả năng của Mosquitto. Bottleneck chỉ xuất hiện ở quy mô lớn hơn nhiều.

---

##### Ứng viên 3 — NanoMQ (EMQ Technologies, C, multi-threaded actor model)

NanoMQ ra đời 2020, implement thuần C, dựa trên NNG's asynchronous I/O với multi-threading Actor Model. Hỗ trợ đầy đủ MQTT 3.1.1 và 5.0, SSL/TLS, và MQTT over QUIC. Startup footprint khoảng 2MB. NanoMQ gia nhập LF Edge Foundation tháng 1/2024, với kế hoạch tích hợp sâu với EdgeX Foundry framework.

| Tiêu chí | Đánh giá | Chi tiết |
|---|---|---|
| Thiết kế cho Edge | ✅ Tốt nhất | Built specifically cho IoT edge — không phải cloud broker thu nhỏ |
| Multi-threading | ✅ Có | Actor model, tận dụng đa core — phù hợp x86 industrial PC |
| Throughput | ✅ Cao hơn Mosquitto | Fan-out: NanoMQ ổn định 250k msg/s; NanoMQ phù hợp nhất cho multi-core platform muốn throughput cao. |
| RAM footprint | ✅ Thấp | ~2MB startup, RAM khi idle ~10–30MB |
| MQTT 5.0 | ✅ Đầy đủ | Bao gồm MQTT over QUIC |
| Built-in bridge | ✅ Có | Bridge sang remote MQTT broker — tiềm năng thay EMQX OT trong tương lai |
| Data persistence | ✅ Có | Offline caching, resume sau reconnect — independent của EMQX OT |
| Rule engine | ⚠ Sơ khai | SQL-based nhưng còn primitive |
| ACL | ✅ Có | Username/password, TLS |
| REST API | ✅ Có | HTTP Management API (port 8081) |
| Fault isolation | ✅ Process riêng | Crash broker không kéo firmware down |
| Restart độc lập | ✅ Có | Systemd unit riêng |
| Độ trưởng thành | ⚠ Trẻ hơn Mosquitto | Ra đời 2020 — chỉ ~4–5 năm tuổi. Một đánh giá thực tế từ production: "NanoMQ có tiềm năng nhưng chưa đủ mature — documentation đôi chỗ lỗi thời, references tính năng chưa được implement trong code, hạn chế về authentication/authorization features." |
| Community | ⚠ Nhỏ hơn Mosquitto | Hệ sinh thái nhỏ hơn, ít tài liệu debug hơn |
| .NET integration | ⚠ IPC qua TCP | Tương tự Mosquitto — kết nối TCP localhost từ `MQTTnet` client |

---

**So sánh tổng hợp 3 ứng viên trong bối cảnh EMS Gateway:**

| Tiêu chí | MQTTnet.Server | Mosquitto | NanoMQ |
|---|---|---|---|
| **Fault isolation** | ❌ Cùng process | ✅ Process riêng | ✅ Process riêng |
| **Restart độc lập** | ❌ Không | ✅ Có | ✅ Có |
| **Độ trưởng thành** | ⚠ Broker mode ít production | ✅ 15 năm, hàng triệu node | ⚠ 4–5 năm, growing |
| **RAM footprint** | ✅ ~10–20MB thêm | ✅ <5MB | ✅ ~10–30MB |
| **Tích hợp .NET** | ✅ In-process | ⚠ TCP IPC | ⚠ TCP IPC |
| **Throughput (50 devices)** | ✅ Đủ | ✅ Đủ | ✅ Dư thừa |
| **Multi-threading** | ✅ .NET threadpool | ❌ Single-thread | ✅ Actor model |
| **Systemd management** | ❌ Không tách được | ✅ Unit file riêng | ✅ Unit file riêng |
| **ACL/Security** | ⚠ Tự implement | ✅ File-based | ✅ Built-in |
| **Built-in bridge** | ❌ Không cần (EMQX OT) | ❌ Không cần | ✅ Có (future-proof) |
| **Documentation** | ⚠ Server mode ít | ✅ Rất đầy đủ | ⚠ Còn lỗi thời |
| **Community support** | ✅ .NET ecosystem | ✅ Lớn nhất | ⚠ Nhỏ hơn |
| **Chi phí vận hành** | ⚠ Cao hơn (gắn firmware) | ✅ Thấp nhất | ✅ Thấp |

---

**Quyết định kiến trúc: Mosquitto là lựa chọn tối ưu**

**Lý do chọn Mosquitto thay vì MQTTnet.Server:**

Vấn đề cốt lõi của `MQTTnet.Server` không phải là hiệu năng hay tính năng — mà là **fault isolation**. Trong môi trường firmware 24/7 tại hiện trường:

```
MQTTnet.Server embedded:
  Bug trong broker session management
  → Memory leak tích lũy theo giờ
  → .NET GC pressure tăng
  → Toàn bộ firmware bị ảnh hưởng
  → Poll Modbus chậm lại, LocalBuffer backpressure
  → Phải restart firmware = mất 10–30 giây thu thập

Mosquitto (process riêng):
  Bug trong broker session management
  → systemd restart mosquitto (< 2 giây)
  → MQTTnet client firmware reconnect tự động
  → Firmware không bị ảnh hưởng, poll tiếp tục
  → Thiết bị MQTT Native reconnect sau <5 giây
```

Firmware Gateway là **core asset** — không được phép bất ổn vì lý do broker. Tách broker ra process riêng là **separation of concerns** đúng nghĩa.

**Lý do chọn Mosquitto thay vì NanoMQ:**

Với quy mô EMS Gateway SME (tối đa 50 thiết bị MQTT Native, ~500 msg/s tổng), throughput multi-threading của NanoMQ là **dư thừa hoàn toàn** — Mosquitto single-threaded xử lý thoải mái. Đổi lại:
- Mosquitto có **15 năm production history** — NanoMQ chỉ 4–5 năm
- Mosquitto có **documentation đầy đủ và chính xác** — NanoMQ còn một số lỗi thời
- Mosquitto là **lựa chọn thấp rủi ro** cho team 1–2 người vận hành tại SME Việt Nam
- Khi nào thiết bị MQTT Native vượt 200 devices hoặc throughput > 10.000 msg/s → migrate sang NanoMQ chỉ cần thay `mosquitto` systemd unit, không thay đổi firmware

**Kiến trúc cuối cùng cho South-facing broker:**

```
[Tầng 2A — IIoT Gateway machine]
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│  Process 1: Firmware .NET 8                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  MqttNativeAdapter (MQTTnet client)                 │   │
│  │    → subscribe localhost:1883 (Mosquitto)           │   │
│  │    → nhận message từ thiết bị MQTT trường           │   │
│  │  SparkplugEncoder / LocalBuffer                     │   │
│  │    → publish localhost:1883 (Mosquitto)             │   │
│  │    → Mosquitto bridge → EMQX OT (Tầng 2B)          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  Process 2: Mosquitto (systemd unit riêng)                  │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Port 1883 — nhận từ:                               │   │
│  │    a) Thiết bị MQTT Native Luồng A (publish vào đây)│   │
│  │    b) Firmware .NET (SparkplugEncoder publish)       │   │
│  │  Bridge → EMQX OT Tầng 2B (port 1884 LAN OT)       │   │
│  │  ACL: whitelist IP range thiết bị trường            │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘

[Thiết bị MQTT Native Luồng A]
    publish → Mosquitto port 1883 (trên Gateway machine)
                  ↓ (Mosquitto bridge)
           [EMQX OT — Tầng 2B, server riêng]
                  ↓ (EMQX Bridge TLS)
           [EMQX IT — Tầng 3]
```

**Lợi ích bổ sung của kiến trúc này:** Mosquitto đang chạy sẵn trên Gateway machine thực ra **đã giải quyết luôn** bài toán "EMQX OT down". Firmware publish lên Mosquitto local → Mosquitto bridge sang EMQX OT. Khi EMQX OT down:
- Mosquitto vẫn nhận publish từ firmware và thiết bị MQTT Native
- Mosquitto buffer local (SQLite persistence) cho đến khi EMQX OT phục hồi
- Firmware LocalBuffer (Module 2A-8) = lớp backup thứ hai cho Sparkplug B payload

**Hai lớp buffer bổ sung nhau:**

```
[Firmware LocalBuffer — Module 2A-8]
  Buffer khi: Mosquitto local restart (<2 giây)
  Capacity: 72h SQLite trên eMMC Gateway

[Mosquitto local persistence/bridge buffer]
  Buffer khi: EMQX OT (Tầng 2B) down
  Capacity: cấu hình queue_size trong mosquitto.conf
  Thời gian: đến khi EMQX OT phục hồi
```

**Cấu hình Mosquitto tối thiểu cho Gateway:**

```conf
# /etc/mosquitto/mosquitto.conf

# Listener South-facing (thiết bị trường + firmware)
listener 1883 0.0.0.0
allow_anonymous false
password_file /etc/mosquitto/passwd

# ACL: firmware được publish Sparkplug B
# Thiết bị MQTT Native được publish topic riêng
acl_file /etc/mosquitto/acl

# Bridge North-facing → EMQX OT (Tầng 2B)
connection emqx_ot_bridge
address 10.0.x.x:1884           ← EMQX OT server Tầng 2B
topic spBv1.0/# out 1           ← forward Sparkplug B lên Tầng 2B
topic ems/config/# in 1         ← nhận Config Channel từ Tầng 2B (G2)
bridge_cafile /etc/mosquitto/ca.crt
bridge_certfile /etc/mosquitto/gw.crt
bridge_keyfile /etc/mosquitto/gw.key

# Persistence (buffer khi EMQX OT down)
persistence true
persistence_location /var/lib/mosquitto/
max_queued_messages 10000
```

**Deployment artifacts bổ sung vào `deploy/`:**

```
deploy/
├── mosquitto/
│   ├── mosquitto.conf          ← template cấu hình
│   ├── acl                     ← ACL rules
│   └── setup-mosquitto.sh      ← install + hardening script
└── systemd/
    ├── ems-gateway.service     ← firmware service
    └── mosquitto.service       ← override: After=mosquitto.service
                                   Restart=on-failure, RestartSec=5
```

**Khi nào nên migrate sang NanoMQ (migration path rõ ràng):**

| Điều kiện | Hành động |
|---|---|
| Thiết bị MQTT Native > 200 devices | Thay `mosquitto` bằng `nanomq` — firmware không đổi |
| Throughput > 10.000 msg/s | Thay `mosquitto` bằng `nanomq` |
| Cần built-in bridge thay EMQX OT | Đánh giá NanoMQ bridge + rule engine |
| QUIC transport cần thiết (mạng OT không ổn định) | NanoMQ MQTT over QUIC |

> **Nguyên tắc bất biến:** South-facing broker (Mosquitto) nhận từ thiết bị trường và firmware, bridge lên North-facing (EMQX OT Tầng 2B). Hai process riêng biệt — độc lập lifecycle, độc lập restart, độc lập fault domain. Firmware .NET chỉ là MQTT client của Mosquitto, không phải host của broker.
...
| I.2 | South-facing MQTT Broker (MQTTnet vs Mosquitto vs NanoMQ) | ✅ **Mosquitto** — fault isolation, 15 năm production, đủ throughput SME; migration path rõ sang NanoMQ khi scale | deploy/ artifacts, Module 2A-3 config |
...
## Chiến lược Mosquitto Bridge → EMQX OT (Tầng 2B)

Phần này đặc tả đầy đủ chiến lược bridge từ Mosquitto (South-facing, Tầng 2A) lên EMQX OT (North-facing, Tầng 2B) — bao gồm các vấn đề kỹ thuật thực tế, edge case với Sparkplug B, và cấu hình production-grade.

---

### Tổng quan luồng dữ liệu đầy đủ sau khi có Mosquitto
...
[Tầng 1 — Thiết bị trường]
  Modbus RTU/TCP / BACnet/IP
       │ poll trực tiếp
       ▼
  [Firmware .NET — Tầng 2A]
  M03→M04→M05→M06→M07
  SparkplugEncoder publish
       │
       ▼ publish spBv1.0/# (QoS 1)
  [Mosquitto — localhost:1883]  ← South-facing broker, process riêng
       │                             nhận từ:
       │  ┌─ Firmware .NET (Sparkplug B DDATA/DBIRTH/DDEATH)
       │  └─ Thiết bị MQTT Native Luồng A (raw JSON topic)
       │
       ▼ Mosquitto Bridge (TLS, QoS 1, cleansession=false)
  [EMQX OT — Tầng 2B, IP:1883/8883]  ← North-facing broker
       │ Store-and-Forward 72h
       ▼ MQTT Bridge TLS DMZ port 8883
  [EMQX IT — Tầng 3]
       │ Sparkplug B Decoder
       ▼ InfluxDB + PostgreSQL + Analytics

[Config Channel G2 — chiều ngược]:
  EMQX IT → Bridge → EMQX OT → Mosquitto Bridge (in) → Firmware
[DCMD G3 — chiều ngược]:
  EMQX IT → Bridge → EMQX OT → Mosquitto Bridge (in) → Firmware M09
```

---

### Vấn đề kỹ thuật quan trọng khi bridge Sparkplug B qua Mosquitto

#### Vấn đề 1 — `cleansession` và hành vi khi reconnect

Đây là điểm phức tạp nhất và hay gây mất dữ liệu nhất trong production.

**`cleansession=false` (persistent session):**
- EMQX OT giữ queue cho Mosquitto bridge khi bridge mất kết nối
- Khi reconnect, EMQX OT gửi lại toàn bộ queue → đảm bảo không mất message
- **Rủi ro thực tế đã quan sát:** Khi bridge reconnect với `cleansession=false` và có retain message, Mosquitto có thể gửi lại duplicate message nhiều lần — đã xác nhận qua GitHub issue #2248. Nguyên nhân: retained message trên local broker bị re-forward khi bridge reconnect.

**`cleansession=true`:**
- EMQX OT không giữ queue cho bridge
- Message publish trong khi bridge down → mất khi reconnect
- Không phù hợp — Firmware LocalBuffer (Module 2A-8) buffer phía local, nhưng phía EMQX OT không biết Mosquitto đã mất bao nhiêu message

**Giải pháp đúng — tách biệt `cleansession` và `local_cleansession`:**

```conf
# Bridge connection đến EMQX OT
connection emqx_ot
address 10.0.x.x:8883

# cleansession=false: EMQX OT giữ queue khi bridge offline
cleansession false

# local_cleansession=false: Mosquitto local giữ subscription
# khi EMQX OT restart (không mất subscribe list)
local_cleansession false
```

**Kết hợp với Firmware LocalBuffer (Module 2A-8):**
- Firmware LocalBuffer buffer khi **Mosquitto** bị down hoặc restart
- Mosquitto bridge queue (persistent) buffer khi **EMQX OT** bị down
- Hai lớp buffer độc lập, bổ sung nhau — không cùng fail

---

#### Vấn đề 2 — Retain flag và Sparkplug B

**Sparkplug B spec quy định rõ:**
- `DBIRTH`: **retain=true** — EMQX OT lưu Birth Certificate, subscriber mới kết nối nhận ngay
- `DDEATH`: **retain=true** — lưu trạng thái Death
- `DDATA`: **retain=false** — không retain, chỉ forward real-time

**Vấn đề khi Mosquitto bridge forward retain message:**

Mặc định Mosquitto bridge **không** preserve retain flag khi forward — message đến EMQX OT sẽ không được retain. Phải cấu hình `try_private true` để Mosquitto báo với EMQX OT biết nó là bridge, giúp retain propagate đúng.

```conf
# Bắt buộc để retain flag được giữ nguyên khi bridge
try_private true
```

**Nếu EMQX OT không hỗ trợ `try_private`:** Sử dụng topic-level bridge rule với retain flag explicit:

```conf
# DBIRTH và DDEATH — retain=true phải được preserve
topic spBv1.0/+/DBIRTH/# out 1
topic spBv1.0/+/DDEATH/# out 1
# DDATA — non-retain, QoS 1
topic spBv1.0/+/DDATA/# out 1
```

---

#### Vấn đề 3 — Duplicate message khi replay

Khi EMQX OT down và phục hồi, hai nguồn message có thể replay cùng lúc:
1. **Firmware LocalBuffer** replay Sparkplug B payload lên Mosquitto
2. **Mosquitto bridge queue** (persistent session) forward lại queue cũ lên EMQX OT

Nếu cả hai replay cùng một khoảng thời gian → **duplicate payload** đến EMQX OT → EMQX OT forward đến Tầng 3 → duplicate trong InfluxDB.

**Giải pháp — Deduplication đã có sẵn:**
- Deduplication key `(seq_number + device_id + timestamp_utc_ns)` được thiết kế trong Module 2A-8 và Tầng 2B (`StoreForward`)
- Firmware replay và Mosquitto replay đều mang cùng Sparkplug B payload nguyên vẹn với cùng `seq_number + timestamp_ns`
- Tầng 3 (Business Server) reject duplicate theo key này — không ghi 2 lần vào InfluxDB

**Bổ sung cần làm rõ:** Tầng 3 phải implement deduplication window đủ rộng (ít nhất 72 giờ + replay time) để cover toàn bộ kịch bản outage dài.

---

#### Vấn đề 4 — `queue_qos0_messages` và throughput burst

Mặc định Mosquitto **không** queue QoS 0 message khi bridge offline. Sparkplug B dùng QoS 1 nên không bị vấn đề này. Tuy nhiên cần cấu hình giới hạn queue hợp lý:

```conf
# Số message tối đa trong queue khi bridge offline
# 50 devices × 10 msg/s × 72h = 129.600.000 — quá lớn
# Thực tế: Firmware LocalBuffer đã cover — Mosquitto bridge queue chỉ cần cover
# khoảng thời gian ngắn trước khi Firmware LocalBuffer kịch hoạt (< 10 giây)
# → giới hạn hợp lý: 10.000 message (~16 phút với 10 msg/s)
max_queued_messages 10000

# Giới hạn bytes để tránh OOM nếu payload lớn
max_queued_bytes 52428800    # 50MB
```

**Lý do không cần queue lớn hơn:** Firmware LocalBuffer (Module 2A-8) là lớp buffer chính cho outage dài (72h). Mosquitto bridge queue chỉ là lớp buffer phụ cho khoảng lag giữa "Mosquitto nhận từ firmware" và "bridge forward thành công lên EMQX OT".

---

#### Vấn đề 5 — Config Channel và DCMD (chiều ngược IT → Gateway)

Mosquitto bridge cần nhận message từ EMQX OT cho hai mục đích:
- **Config Channel (G2):** topic `ems/config/{gateway_id}/request`
- **DCMD Command (G3):** topic `spBv1.0/{group}/{gateway_id}/DCMD`

```conf
# Chiều vào (EMQX OT → Mosquitto local → Firmware subscribe)
topic ems/config/# in 1
topic spBv1.0/+/+/DCMD in 1    # G3 only

# Lưu ý: "in" direction = Mosquitto subscribe từ EMQX OT
# Firmware .NET subscribe từ Mosquitto local để nhận
```

**Lưu ý quan trọng về `try_private` và `in` direction:**
Khi `try_private=true`, EMQX OT biết đây là bridge và không loop message. Nhưng với EMQX v5.x, cần kiểm tra xem EMQX OT có support `bridge_mode` connection hay không — một số version EMQX yêu cầu thêm authentication rule cho bridge client.

---

### Cấu hình Mosquitto production-grade đầy đủ

```conf
# /etc/mosquitto/conf.d/ems-gateway.conf
# EMS IIoT Gateway — South-facing MQTT Broker
# Phiên bản: production G1/G2/G3
# Cập nhật: [ngày]

# ════════════════════════════════════════
# LISTENER — South-facing (thiết bị trường)
# ════════════════════════════════════════
listener 1883 0.0.0.0
# Không allow_anonymous — mọi client phải authenticate
allow_anonymous false
password_file /etc/mosquitto/passwd

# ACL — phân quyền per client
acl_file /etc/mosquitto/acl

# ════════════════════════════════════════
# PERSISTENCE — buffer khi restart
# ════════════════════════════════════════
persistence true
persistence_location /var/lib/mosquitto/
# Autosave mỗi 30 giây (tránh mất dữ liệu khi crash)
autosave_interval 30
# Persistent client expiry — xóa session cũ sau 7 ngày không reconnect
persistent_client_expiration 7d

# Queue limits (áp dụng cho mọi client bao gồm bridge)
max_queued_messages 10000
...
# ════════════════════════════════════════
# BRIDGE → EMQX OT (Tầng 2B)
# ════════════════════════════════════════
connection emqx_ot_bridge
# G1/G2 co-located: localhost:1883 (EMQX OT cùng máy)
# G3 tách máy: IP của EMQX OT server + port 8883 (TLS)
address 10.0.x.x:8883

# Protocol version — MQTT 5.0 nếu EMQX OT hỗ trợ, fallback 3.1.1
bridge_protocol_version mqttv50

# Client ID duy nhất cho từng Gateway — không được trùng
remote_clientid ems-gw-{machine_id}-bridge

# Credentials cho EMQX OT (mTLS hoặc username/password)
bridge_cafile /etc/mosquitto/certs/ca.crt
bridge_certfile /etc/mosquitto/certs/gw-{machine_id}.crt
bridge_keyfile /etc/mosquitto/certs/gw-{machine_id}.key

# Persistent session — EMQX OT giữ queue khi bridge offline
cleansession false
local_cleansession false

# try_private — báo EMQX OT biết đây là bridge (không loop, retain propagate đúng)
try_private true

# Keepalive và reconnect
keepalive_interval 60
restart_timeout 5 30      # min 5s, max 30s giữa các lần reconnect
# start_type: automatic = kết nối ngay khi mosquitto start
start_type automatic

# ─────────────────────────────────────────
# TOPIC RULES — Outgoing (Mosquitto → EMQX OT)
# ─────────────────────────────────────────
# NBIRTH / NDEATH (Node-level Sparkplug B)
topic spBv1.0/+/NBIRTH out 1
topic spBv1.0/+/NDEATH out 1

# DBIRTH — retain=true phải được preserve (try_private=true đảm bảo điều này)
topic spBv1.0/+/DBIRTH/# out 1

# DDATA — non-retain, QoS 1
topic spBv1.0/+/DDATA/# out 1

# DDEATH — retain=true phải preserve
topic spBv1.0/+/DDEATH/# out 1

# ─────────────────────────────────────────
# TOPIC RULES — Incoming (EMQX OT → Mosquitto)
# ─────────────────────────────────────────
# Config Channel (G2): Business Server → Firmware
topic ems/config/# in 1

# DCMD Command Channel (G3): Business Server → M09 Firmware
topic spBv1.0/+/+/DCMD in 1
```
...
```
# Firmware .NET — publish Sparkplug B, subscribe Config/DCMD
user ems-firmware
topic write spBv1.0/#
topic read ems/config/#
topic read spBv1.0/+/+/DCMD

# Thiết bị MQTT Native Luồng A — chỉ publish topic riêng
# (mỗi device có user riêng, pattern username)
pattern write factory/%u/#

# Bridge client — read toàn bộ để forward
user ems-bridge
topic read #
topic write ems/config/#
topic write spBv1.0/+/+/DCMD
```

---

### Luồng xử lý khi EMQX OT down (Kịch bản G3 tách máy)

```
Bình thường:
  Firmware → Mosquitto (localhost) → Bridge → EMQX OT ✅

EMQX OT down:
  Step 1: Mosquitto bridge phát hiện TCP disconnect
  Step 2: Mosquitto enqueue outgoing messages vào persistent queue
          (giới hạn max_queued_messages = 10.000)
  Step 3: Firmware tiếp tục publish lên Mosquitto local ✅
          (Mosquitto vẫn running — không ảnh hưởng firmware)
  Step 4: Khi Mosquitto local queue đầy (10.000 msg):
          Firmware LocalBuffer (Module 2A-8) nhận backpressure
          → enqueue vào SQLite 72h ✅
  Step 5: Thiết bị MQTT Native Luồng A tiếp tục publish
          vào Mosquitto ✅ (không biết EMQX OT đang down)

EMQX OT phục hồi:
  Step 1: Mosquitto bridge reconnect tự động (restart_timeout)
  Step 2: Mosquitto drain queue (10.000 msg) → EMQX OT
          Rate-limit tự nhiên bởi TCP throughput
  Step 3: Firmware LocalBuffer replay (rate-limited 500 msg/s
          theo cấu hình bổ sung I.7)
  Step 4: Tầng 3 deduplication theo (seq+device_id+timestamp_ns)
          → không ghi duplicate vào InfluxDB ✅
```

---

### Điểm kiểm tra (Verification Checklist) sau khi deploy

| Kiểm tra | Lệnh | Kết quả mong đợi |
|---|---|---|
| Bridge đang kết nối | `mosquitto_sub -v -t '$SYS/broker/connection/emqx_ot_bridge/state'` | `1` |
| DBIRTH được retain trên EMQX OT | `mosquitto_sub -h [EMQX_OT_IP] -t 'spBv1.0/#' --retained-only` | DBIRTH message |
| Queue size khi offline | `mosquitto_sub -v -t '$SYS/broker/messages/stored'` | Tăng khi EMQX OT down |
| ACL hoạt động | Thử publish topic không trong whitelist | Connection rejected |
| TLS verify | `openssl s_client -connect [EMQX_OT_IP]:8883 -CAfile ca.crt` | Verify OK |
| Persistent session | Restart mosquitto → check EMQX OT vẫn giữ queue | Queue intact |

---

### Sơ đồ cập nhật kiến trúc Tầng 2A với Mosquitto
...
│                                                                     │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  Process 1: Mosquitto (systemd: mosquitto.service)            │  │
│  │  Port 1883 — South-facing broker                              │  │
│  │  ┌─────────────────┐   ┌─────────────────────────────────┐   │  │
│  │  │ Nhận từ:        │   │ Bridge → EMQX OT (Tầng 2B)      │   │  │
│  │  │ • Firmware .NET │   │ TLS port 8883                   │   │  │
│  │  │ • MQTT Native   │   │ cleansession=false              │   │  │
│  │  │   thiết bị (A)  │   │ try_private=true                │   │  │
│  │  │                 │   │ Retain propagate: DBIRTH/DDEATH │   │  │
│  │  │ Persistent queue│   │ Queue: 10.000 msg / 50MB        │   │  │
│  │  │ 10.000 msg      │   │ restart_timeout: 5–30s          │   │  │
│  │  └─────────────────┘   └─────────────────────────────────┘   │  │
│  └───────────────────────────────────────────────────────────────┘  │
│                    ▲                    ▲                            │
│        publish     │                   │  subscribe                 │
...
│                    │                   │                            │
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  Process 2: Firmware .NET 8 (systemd: ems-gateway.service)    │  │
│  │  After=mosquitto.service                                      │  │
│  │                                                               │  │
│  │  MqttNativeAdapter ──subscribe──► Mosquitto:1883              │  │
│  │  (thiết bị MQTT Luồng A)                                      │  │
│  │                                                               │  │
│  │  SparkplugEncoder ──publish──► Mosquitto:1883                 │  │
│  │  (DDATA/DBIRTH/DDEATH QoS 1)                                  │  │
│  │                                                               │  │
│  │  LocalBuffer (Module 2A-8) ─► SQLite 72h                      │  │
│  │  (buffer khi Mosquitto down < 2s)                             │  │
│  └───────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
### Systemd dependency đảm bảo thứ tự khởi động đúng
...
Requires=mosquitto.service    # ← Gateway không start nếu Mosquitto chưa running

[Service]
User=ems-gateway
Group=ems-gateway dialout
...
> **Lưu ý:** `Requires=mosquitto.service` đảm bảo nếu Mosquitto bị kill, systemd cũng stop firmware và restart cả hai cùng nhau — tránh tình trạng firmware chạy mà không có broker để publish.

---

## Phân tích & Phản biện Yêu cầu Bổ sung — Nhóm "Bullet-proof" (v1.7)

Đây là 6 yêu cầu từ góc nhìn vận hành thực chiến, xuất phát từ kinh nghiệm node edge thực tế với điện nhà máy mất bất cứ lúc nào và mạng OT nhiễu loạn. Phân tích dưới đây đánh giá từng yêu cầu với mức độ phản biện kỹ thuật cao nhất.

---

### ✅ Yêu cầu 1 — Modbus Register Coalescing (Chấp nhận — bổ sung vào Module 2A-3)

**Phản biện:** Yêu cầu này **hoàn toàn đúng và bị thiếu** trong thiết kế hiện tại. Thiết kế hiện tại của `PollAsync(device, registers)` gọi từng register riêng lẻ — trên RS485 9600 baud, mỗi request/response cycle tốn ~10–50ms. Với 10 register rải rác, tổng poll time có thể lên 200–500ms/cycle, chiếm toàn bộ `poll_cycle_ms=500` mà không còn thời gian cho thiết bị khác. Coalescing là kỹ thuật chuẩn mực của mọi industrial gateway production-grade.

**Tuy nhiên, cần phản biện một chi tiết:** Yêu cầu nêu "gap < N words, mặc định N=10". Thực tế, gap threshold tối ưu phụ thuộc vào thiết bị:
- Một số PLC tốc độ chậm: đọc 100 register liên tiếp tốt hơn 10 request riêng lẻ ngay cả khi có gap 50 register rỗng
- Một số đồng hồ điện cũ: có giới hạn max_count per request (thường 125 register/request theo Modbus spec FC03)

**Thiết kế bổ sung vào Module 2A-3:**

```
Register Coalescing Algorithm (tại DeviceTemplate load time, không runtime):

Input: danh sách RegisterDefinitionDto của một device, đã sort theo address
Output: danh sách CoalescedBlock[] — mỗi block là một FC03 request

Thuật toán:
  1. Sort registers tăng dần theo address
  2. Duyệt tuần tự:
     - Nếu (current.address - last_block_end) <= coalesce_gap_words
       VÀ (current.address - block_start) <= max_registers_per_request (125)
       → Mở rộng block hiện tại
     - Ngược lại → Đóng block hiện tại, mở block mới
  3. Trả danh sách CoalescedBlock[]

CoalescedBlock:
  - start_address: int
  - count: int          ← số word cần đọc (span = end_address - start + 1)
  - register_offsets[]  ← offset của từng tag trong raw byte array
  - data_types[]        ← float32/uint16/int32... để parse đúng

Sau khi đọc block:
  raw_bytes[] → slice theo register_offsets → parse từng tag → RawTagDto[]
```

Config bổ sung trong `DeviceTemplateDto`:
```json
"modbus_coalescing": {
  "enabled": true,
  "max_gap_words": 10,        // gap tối đa giữa 2 register để gom
  "max_registers_per_block": 100  // giới hạn an toàn (< 125 per Modbus spec)
}
```

**Lợi ích định lượng cho bối cảnh SME:**
- 50 register, rải rác trung bình 3–5 word/gap → ~8–12 block sau coalescing (thay vì 50 request)
- Tốc độ poll tăng 4–6x trên RS485 9600 baud
- Giảm 80% xác suất bus collision khi có nhiều thiết bị trên cùng RS485 segment

**Phân loại trách nhiệm:** Coalescing thuộc `DeviceTemplate` (build lúc load) và `ProtocolAdapter` (execute). `PollingEngine` chỉ nhận `CoalescedBlock[]` từ template mà không biết về thuật toán.

---

### ✅ Yêu cầu 2 — SQLite Corruption Auto-Recovery (Chấp nhận — bổ sung vào Module 2A-8)

**Phản biện:** Yêu cầu này **chính xác và quan trọng**. Thiết kế hiện tại dùng `tmpfs` RAM buffer + SQLite WAL mode trên eMMC, nhưng hoàn toàn **không có xử lý khi SQLite file bị corrupt**. Khi `Microsoft.Data.Sqlite` throw `SqliteException` với `SQLITE_CORRUPT (code 11)`, nếu không catch đúng, `LocalBuffer` sẽ vào crash loop khiến Serilog ghi log liên tục → eMMC bị worn out nhanh hơn chính corrupt.

**Phản biện một điểm trong yêu cầu:** "xóa file .db-wal / .db-shm cũ" — đúng. Nhưng cần thêm: phải **close tất cả connection pool trước khi rename** (SQLite connection pool trong .NET giữ handle file mở) và phải dùng `SqliteConnection.ClearAllPools()` trước khi file operation.

**Phản biện thứ hai:** Không phải chỉ `SQLITE_CORRUPT` mà cần catch toàn bộ nhóm lỗi corruption: `SQLITE_CORRUPT (11)`, `SQLITE_NOTADB (26)`, `SQLITE_IOERR (10)`. Ba mã này đều chỉ file-level damage, không phải query logic error.

**Thiết kế bổ sung vào Module 2A-8:**

```csharp
// Trong SQLite writer loop của LocalBuffer
private async Task ExecuteWithCorruptionRecoveryAsync(Func<Task> operation)
{
    ...
    {
        await HandleCorruptionAsync(ex);
    }
}

private async Task HandleCorruptionAsync(SqliteException ex)
{
    // Bước 1: Close toàn bộ connection trước khi thao tác file
    SqliteConnection.ClearAllPools();
    
    // Bước 2: Rename file hỏng (giữ lại để phân tích sau)
    var timestamp = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
    File.Move(_dbPath, $"{_dbPath}.corrupted_{timestamp}");
    // Xóa WAL và SHM files nếu tồn tại
    File.Delete($"{_dbPath}-wal");
    File.Delete($"{_dbPath}-shm");
    
    // Bước 3: Tạo database mới, trắng hoàn toàn
    await InitializeDatabaseAsync(_dbPath);
    
    // Bước 4: Phát cảnh báo nghiêm trọng
    _logger.LogCritical(
        "SQLite buffer corrupted (code {Code}). "
        + "Buffer reset. Approx {Lost} records lost. "
        + "Corrupted file: {File}",
        ex.SqliteErrorCode, _pendingCount, $"{_dbPath}.corrupted_{timestamp}");
    
    // Bước 5: Publish AlertRaised lên IInternalEventBus
    await _eventBus.PublishAsync(new BufferCorruptedEvent
    {
        Timestamp = DateTimeOffset.UtcNow,
        EstimatedLostRecords = _pendingCount,
        CorruptedFilePath = $"{_dbPath}.corrupted_{timestamp}"
    });
    
    // Reset counter — buffer mới = trống
    _pendingCount = 0;
}
```

**Bổ sung vào Domain Events (Module 2A-1):** Thêm `BufferCorruptedEvent` → UI hiển thị banner CRITICAL màu đỏ trên Dashboard, kèm thông tin số record ước tính bị mất.

**Bổ sung vào Prometheus metrics (Module 2A-9):**
```
gateway_buffer_corruption_total (counter) — số lần corrupt xảy ra kể từ khi boot
```

**Lịch dọn file corrupted:** `AdminService` chạy cleanup job mỗi 24h, xóa file `.corrupted_*` cũ hơn 7 ngày để tránh fill disk.

---

### ✅ Yêu cầu 3 — Automated mTLS Cert Lifecycle (Chấp nhận — bổ sung vào Module 2A-9)

**Phản biện:** Yêu cầu **hoàn toàn đúng** và là lỗ hổng vận hành nghiêm trọng nhất trong triển khai fleet hiện tại. Cert hết hạn là nguyên nhân số 1 gây **silent outage hàng loạt** — tất cả gateway cùng mất kết nối một ngày vào lúc 00:00. Tài liệu Tầng 2B đã đề cập "Script renew tự động 14 ngày trước hết hạn" nhưng Tầng 2A chưa thiết kế cơ chế này.

**Phản biện về lựa chọn công nghệ:** Yêu cầu đề xuất `certbot` (ACME protocol) hoặc `step-ca` client. Cần phân tích:
- `certbot`: Thiết kế cho Let's Encrypt public CA — không phù hợp mạng OT nội bộ không có internet
- `step-ca` client (`step`): Phù hợp hơn vì Business Server (Tầng 3) có thể làm internal CA với `step-ca`; client Gateway dùng `step ca renew` — hoàn toàn offline, không cần internet

**Lựa chọn đúng: `step-ca` client** — nhất quán với thiết kế Tầng 2B đã dùng `step-ca` REST API.

**Phản biện về `systemctl reload mosquitto`:** Đây là điểm kỹ thuật **cực kỳ quan trọng**. `reload` (SIGHUP) với Mosquitto 2.x không phải lúc nào cũng load lại cert file — hành vi phụ thuộc vào compile option và version. Cần dùng `systemctl restart mosquitto` (graceful restart ~2 giây) thay vì `reload` để đảm bảo cert mới được nạp hoàn toàn. Firmware LocalBuffer sẽ buffer trong 2 giây này — không mất dữ liệu.

**Thiết kế bổ sung vào Module 2A-9 (`AdminService`):**

```
IAutoRenewCertManager interface:
  - CheckCertExpiryAsync()    → (days_remaining, cert_path, expiry_date)
  - RenewIfNeededAsync()      → bool (renewed or not)
  - ApplyCertToMosquittoAsync() → restart mosquitto gracefully

Lịch chạy:
  - Check mỗi 24 giờ (background timer)
  - Renew nếu days_remaining < 30 (configurable: CertRenewDaysBeforeExpiry)
  - Sau renew thành công:
    1. Verify cert mới hợp lệ (openssl verify)
    2. Copy cert mới vào /etc/mosquitto/certs/
    3. systemctl restart mosquitto
    4. Chờ 10 giây → verify bridge reconnected
    5. Ghi log INFO + Prometheus metric cert_expiry_days_remaining (gauge)
  - Nếu renew thất bại:
    1. Retry 3 lần (mỗi lần cách 1 giờ)
    2. Sau 3 lần thất bại → phát CertRenewalFailedEvent
    3. Gửi cảnh báo Zalo OA (qua AdminService alert channel độc lập)
    4. Không tự xóa cert cũ — giữ kết nối hiện tại đến khi cert hết hạn

step-ca renewal command:
  step ca renew /etc/mosquitto/certs/gw-{id}.crt \
               /etc/mosquitto/certs/gw-{id}.key \
               --ca-url https://business-server:8443 \
               --root /etc/mosquitto/certs/ca.crt \
               --force
```

**Bổ sung vào Prometheus metrics:**
```
gateway_cert_expiry_days_remaining (gauge) — ngày còn lại trước khi cert hết hạn
gateway_cert_renewal_total (counter)        — số lần renew thành công
gateway_cert_renewal_failed_total (counter) — số lần renew thất bại
```

**Bổ sung vào Dashboard UI:** Hiển thị cert expiry trong card System Resources:
```
TLS Cert     ● Valid  (expires 87 days)    ← xanh > 30 ngày
TLS Cert     ● Renewing...                 ← xanh dương khi đang renew
TLS Cert     ⚠ Expiring soon (12 days)     ← vàng < 30 ngày
TLS Cert     ✖ EXPIRED                     ← đỏ — cần can thiệp ngay
```

---

### ✅ Yêu cầu 4 — A/B Partition OTA (Chấp nhận có phạm vi — bổ sung vào Module 2A-9)

**Phản biện mạnh:** Yêu cầu A/B partitioning **đúng về nguyên tắc** nhưng cần phân tích kỹ trước khi commit vào thiết kế SME.

**Điểm 1 — In-place update hiện tại không phải "cục gạch" nếu thiết kế đúng:**

Thiết kế hiện tại (Module 2A-9) đã có:
- Verify SHA-256 + signature **trước khi apply**
- Rollback tự động nếu firmware mới không startup trong 120 giây
- Graceful shutdown trước khi apply

Rủi ro "mất điện lúc đang copy file" của in-place update được giải quyết bằng:
```
1. Download firmware mới vào /tmp/ems-update/firmware-new (tmpfs hoặc separate partition)
2. Verify SHA-256 + signature
3. systemctl stop ems-gateway
4. cp firmware-new /usr/bin/ems-gateway.new  → atomic rename
5. mv /usr/bin/ems-gateway /usr/bin/ems-gateway.bak
6. mv /usr/bin/ems-gateway.new /usr/bin/ems-gateway
   ← nếu mất điện ở đây: .bak vẫn còn, script restore chạy lúc boot
7. systemctl start ems-gateway
```
Trên Linux ext4/ext4+journaling, `mv` (rename syscall) là **atomic** — không thể half-done. Rủi ro "cục gạch" thực sự gần bằng 0 với cách tiếp cận này.

**Điểm 2 — RAUC/Mender A/B có chi phí thực tế đáng kể:**
- Cần partition layout đặc biệt khi image eMMC (không thể retrofit trên Gateway đã deploy)
- RAUC/Mender cần bootloader support (U-Boot với env partition) — không có trên x86 industrial PC dùng GRUB thông thường
- Overhead vận hành: cần build system tích hợp (Yocto, Buildroot) để tạo update bundle
- Team 1–2 người tại SME không có capacity để vận hành

**Kết luận phản biện:** A/B partition là **over-engineering cho bối cảnh SME x86** nhưng **hợp lý cho ARM embedded với Yocto**. Thiết kế phân nhánh:

| Hardware | OTA Strategy | Lý do |
|---|---|---|
| x86 Industrial PC (G1/G2/G3 SME) | **Atomic in-place + backup binary** | GRUB không cần env partition đặc biệt; mv atomic đủ an toàn |
| ARM SBC (Raspberry Pi, i.MX8) | **RAUC A/B** | U-Boot hỗ trợ, eMMC có thể partition khi flash image ban đầu |

**Bổ sung vào Module 2A-9 — Atomic In-place OTA (x86):**

```bash
# /usr/lib/ems-gateway/ota-apply.sh
# Chạy bởi firmware sau khi verify xong

set -euo pipefail

NEW_BIN="$1"   # đường dẫn binary mới đã verify
CURRENT="/usr/bin/ems-gateway"
BACKUP="/usr/bin/ems-gateway.bak"

# Backup binary hiện tại
cp "$CURRENT" "$BACKUP"

# Atomic replace (rename syscall — không thể interrupted giữa chừng)
mv "$NEW_BIN" "$CURRENT"

# Restart service
systemctl restart ems-gateway

# Health check: chờ tối đa 120s
timeout 120 bash -c 'until systemctl is-active --quiet ems-gateway; do sleep 2; done'

# Nếu service không lên sau 120s → restore backup
if ! systemctl is-active --quiet ems-gateway; then
    mv "$BACKUP" "$CURRENT"
    systemctl start ems-gateway
    exit 1
fi

# Thành công — xóa backup sau 24h (cron job)
echo "$BACKUP" >> /var/lib/ems-gateway/pending-cleanup.txt
```

**Bổ sung vào Module 2A-9 — RAUC Integration (ARM, tùy chọn G3):**

```
Khi hardware là ARM + RAUC available:
  IAdminService.OtaStrategy = RaucAbPartition
  
  Flow:
  1. Download bundle (.raucb) vào /tmp/
  2. Verify signature với RAUC bundle signing key
  3. rauc install /tmp/firmware.raucb
     → RAUC ghi vào partition B (không touch partition A)
  4. rauc status → verify install OK
  5. reboot  (bootloader chọn partition B)
  6. Boot partition B → health check 5 phút
     - MQTT connected? → rauc mark-good → commit
     - MQTT không connect? → bootloader auto rollback partition A

Feature flag: "OtaStrategy": "InPlaceAtomic" | "RaucAB"
```

---

### ✅ Yêu cầu 5 — Setup Wizard / OOBE (Chấp nhận — màn hình UI mới)

**Phản biện:** Yêu cầu **hoàn toàn đúng** từ góc nhìn vận hành. Không có OOBE là nguyên nhân thường gặp nhất của việc "deploy xong nhưng Gateway không gửi dữ liệu lên" — kỹ sư quên cấu hình một trong bốn bước. Đặc biệt với đội IT 1–2 người tại SME, chuẩn hóa quy trình triển khai là thiết yếu.

**Phản biện một điểm:** "Phải hoàn thành Wizard mới được truy cập Dashboard" — đúng, nhưng cần **exception cho diagnostic mode**: nếu Gateway đã cấu hình rồi bị factory reset do sự cố, kỹ sư cần truy cập Dashboard để đọc log ngay, không thể bị block bởi Wizard trong tình huống khẩn cấp. Giải pháp: nút "Skip (Emergency Access)" với log WARN + ghi nhận không qua Wizard.

**Thiết kế màn hình OOBE:**

```
┌──────────────────────────────────────────────────────────────────────┐
│  🏭 EMS GATEWAY — SETUP WIZARD                    Bước [1] / [4]    │
│  Thiết lập lần đầu — Vui lòng hoàn thành 4 bước trước khi vận hành  │
├──────────────────────────────────────────────────────────────────────┤
│                                                                       │
│  ●──────○────────○────────○                                          │
│  Mật khẩu  IP   MQTT Broker  NTP                                     │
│                                                                       │
│  ┌─────────────────────────────────────────────────────────────────┐ │
│  │  BƯỚC 1 / 4 — Đổi mật khẩu Admin mặc định                     │ │
│  │                                                                 │ │
│  │  ⚠ Mật khẩu mặc định đang được sử dụng.                       │ │
│  │  Bắt buộc phải đổi trước khi tiếp tục.                        │ │
│  │                                                                 │ │
│  │  Mật khẩu mới    [                    ] (tối thiểu 12 ký tự)  │ │
│  │  Xác nhận        [                    ]                        │ │
│  │                                                                 │ │
│  │  Yêu cầu: ≥12 ký tự, có chữ hoa, số và ký tự đặc biệt        │ │
│  └─────────────────────────────────────────────────────────────────┘ │
│                                                                       │
│  [Skip — Emergency Access ⚠]              [Tiếp theo →]             │
└──────────────────────────────────────────────────────────────────────┘

Bước 2 — IP tĩnh LAN OT:
  Interface  [eth0 ▼]
  IP Address [192.168.10.___]   Subnet [255.255.255.0]
  Gateway    [192.168.10.1  ]
  [Test reachability → ping gateway IP]  ● Reachable

Bước 3 — MQTT Broker (EMQX OT):
  Broker Host [10.0.x.x        ]   Port [8883]
  TLS         [● Enabled]
  [Test Connection → ● Connected  (23ms)]

Bước 4 — NTP Sync:
  NTP Server  [10.0.x.x (Business Server)]
  Hiện tại    : 2024-11-15  14:32:01  UTC+7
  NTP Status  : ● Synced  Drift: +0.12s
  [Force Sync Now]
  [✓ Hoàn thành Setup]
```

**Backend logic:**
- Flag `FirstRunCompleted` lưu trong `/etc/ems-gateway/setup.flag`
- Nếu flag chưa có → redirect toàn bộ request về `/wizard`
- Sau khi hoàn thành Wizard → tạo flag → redirect về Dashboard
- Factory Reset xóa flag → Wizard xuất hiện lại

**Tích hợp với `appsettings.json`:** Wizard ghi trực tiếp vào `appsettings.json` thông qua API `PUT /api/config/network`, `PUT /api/config/mqtt`, `PUT /api/config/ntp` đã có sẵn trong AdminService.

---

### ✅ Yêu cầu 6 — Network Diagnostics Tool (Chấp nhận — bổ sung vào UI Maintenance)

**Phản biện:** Yêu cầu **hoàn toàn đúng và thực tế**. "Modbus TCP timeout" có thể do 4 nguyên nhân khác nhau (đứt cáp, sai IP, PLC treo, firewall block) — không có network diagnostics, kỹ sư mù tịt. Việc yêu cầu kỹ sư SSH vào Linux để gõ `ping` là không khả thi với đội bảo trì SME.

**Phản biện kỹ thuật — stream via SignalR:** Cần lưu ý `traceroute` và `ping` có thể chạy 30–60 giây. Phải implement timeout cứng và allow cancel từ UI. Không được để kỹ sư chạy nhiều diagnostic cùng lúc (race condition stdout stream).

**Phản biện bảo mật:** `ping` và `traceroute` là an toàn. Nhưng cần giới hạn: không cho phép chạy lệnh tùy ý — chỉ whitelist 3 lệnh được phép: `ping`, `traceroute`, `tcp-port-check`. Không dùng `Process.Start(userInput)` trực tiếp.

**Thiết kế UI — sub-tab Network Diagnostics trong Maintenance:**

```
┌──────────────────────────────────────────────────────────────────────┐
│  MAINTENANCE  /  NETWORK DIAGNOSTICS                                 │
├──────────────────────────────────────────────────────────────────────┤
│                                                                       │
│  Target IP/Host  [192.168.10.51     ]   Port  [502   ]              │
│                                                                       │
│  [📡 Ping ICMP]   [🔍 Traceroute]   [🔌 TCP Port Check]             │
│                                                                       │
│  ─────────────────────────────────────────────────────────────────  │
│  KHAI NHANH THIẾT BỊ:                                               │
│  [meter-main-01 (10.0.1.11:502)]  [vfd-comp-01 (10.0.1.51:502)]    │
│  → Click để auto-fill IP/Port từ Device Template                    │
│  ─────────────────────────────────────────────────────────────────  │
│                                                                       │
│  OUTPUT (stream realtime):                                           │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │  [14:32:01] Ping 192.168.10.51 (4 packets):                  │   │
│  │  64 bytes from 192.168.10.51: icmp_seq=1 ttl=64 time=0.8ms  │   │
│  │  64 bytes from 192.168.10.51: icmp_seq=2 ttl=64 time=0.9ms  │   │
│  │  64 bytes from 192.168.10.51: icmp_seq=3 ttl=64 time=1.1ms  │   │
│  │  64 bytes from 192.168.10.51: icmp_seq=4 ttl=64 time=0.8ms  │   │
│  │  ─────────────────────────────────                           │   │
│  │  ● Reachable — avg 0.9ms, 0% packet loss                     │   │
│  │                                                               │   │
│  │  [14:32:05] TCP Port Check 192.168.10.51:502                 │   │
...
**Backend implementation (API `POST /api/diagnostics/network`):**

```csharp
public enum NetworkDiagnosticType { Ping, Traceroute, TcpPortCheck }

// Whitelist — không cho phép lệnh tùy ý
private static string BuildCommand(NetworkDiagnosticType type, string ip, int? port)
    => type switch
    {
        NetworkDiagnosticType.Ping =>
            $"ping -c 4 -W 2 {ValidateIp(ip)}",
        NetworkDiagnosticType.Traceroute =>
            $"traceroute -w 2 -m 15 {ValidateIp(ip)}",
        NetworkDiagnosticType.TcpPortCheck =>
            $"timeout 3 bash -c 'cat < /dev/null > /dev/tcp/{ValidateIp(ip)}/{ValidatePort(port)}'",
        _ => throw new ArgumentException("Unknown diagnostic type")
    };

// Stream output qua SignalR hub "NetworkDiagnosticHub"
// Timeout cứng: ping=15s, traceroute=60s, tcp=5s
// Chỉ 1 diagnostic chạy đồng thời (SemaphoreSlim(1))
```

**Tính năng "Quick Device":** Nút quick-fill IP/Port từ Device Template giúp kỹ sư không cần nhớ IP — click vào tên thiết bị là auto-fill và chạy TCP Port Check ngay. Đây là UX cực kỳ hữu ích tại hiện trường.

---

### Tóm tắt phân loại 6 yêu cầu

| # | Yêu cầu | Quyết định | Vị trí bổ sung |
|---|---|---|---|
| 1 | Modbus Register Coalescing | ✅ Bổ sung đầy đủ | Module 2A-3, 2A-2 (build-time) |
| 2 | SQLite Corruption Auto-Recovery | ✅ Bổ sung + mở rộng error codes | Module 2A-8, 2A-1 (event mới) |
| 3 | Automated mTLS Cert Lifecycle | ✅ Bổ sung với `step-ca`; sửa `reload` → `restart` | Module 2A-9 |
| 4 | A/B Partition OTA | ✅ Có điều kiện: Atomic in-place (x86) + RAUC (ARM tùy chọn) | Module 2A-9, deploy/ |
| 5 | Setup Wizard / OOBE | ✅ Bổ sung + thêm Emergency Skip | UI màn hình mới |
| 6 | Network Diagnostics Tool | ✅ Bổ sung + whitelist lệnh + Quick Device | UI Maintenance tab mới |

> **Nhận xét tổng thể:** Cả 6 yêu cầu đều xuất phát từ kinh nghiệm vận hành thực chiến và không có yêu cầu nào cần bác bỏ. Hai điểm phản biện quan trọng nhất: (1) A/B partition chỉ phù hợp ARM/Yocto, không phải x86 SME — cần phân nhánh chiến lược OTA; (2) `systemctl reload mosquitto` không đảm bảo cert reload, phải dùng `restart` — lỗi này trong production sẽ khiến cert mới không được áp dụng cho đến lần restart tiếp theo.

---

*Phiên bản 1.7 — Bổ sung 6 yêu cầu "Bullet-proof": Modbus Register Coalescing (2A-3, build-time algorithm); SQLite Corruption Auto-Recovery với đầy đủ error codes SQLITE_CORRUPT/NOTADB/IOERR, ClearAllPools, BufferCorruptedEvent (2A-8); Automated mTLS Cert Lifecycle với step-ca, sửa reload→restart, Prometheus cert metrics (2A-9); OTA phân nhánh Atomic in-place (x86) vs RAUC A/B (ARM tùy chọn) (2A-9, deploy/); Setup Wizard OOBE 4 bước với Emergency Skip flag (UI mới); Network Diagnostics Tool với whitelist lệnh, SignalR stream, Quick Device fill (UI Maintenance).*  
