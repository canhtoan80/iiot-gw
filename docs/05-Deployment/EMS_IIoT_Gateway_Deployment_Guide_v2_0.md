...
# ─── Listener (thiết bị trường + firmware) ─────────────────────────
listener 1883 127.0.0.1     # CHỈ localhost — không expose ra ngoài
...
autosave_interval 30         # flush to disk mỗi 30s
max_queued_messages 10000
max_queued_bytes 52428800    # 50MB

# ─── Bridge → EMQX OT (Tầng 2B) ───────────────────────────────────
connection emqx_ot_bridge
address 192.168.10.10:8883   # EMQX OT host:port
...
# Session — persistent để EMQX OT giữ queue khi bridge offline
cleansession false
local_cleansession false

# try_private: báo EMQX biết là bridge → retain messages propagate đúng
try_private true

# Kết nối
keepalive_interval 60
start_type automatic
restart_timeout 5 30         # retry sau 5s, tăng dần lên 30s

# ─── Topic Rules: Outgoing (Mosquitto → EMQX OT) ───────────────────
# Sparkplug B messages — retain cho DBIRTH/DDEATH
...
# DCMD Command Channel (G3 - chỉ bật khi feature flag = true)
...
□ Bước 2: MQTT bridge connected?
  ...
  → Nếu 0: kiểm tra network đến EMQX OT, kiểm tra cert
